...
import { isCancelledJobMessage } from "../lib/jobPolling";
  ...
  const timer = window.setInterval(async () => {
    try {
      ...
      if (s.status === "done") {
        ...
        opts.onSuccess?.();
      } else if (isCancelledJobMessage(s.error)) {
        toastSuccess?.(`${opts.label} cancelled`);
      } else {
...
/** Jobs still polling in the background (any project). */
export function runningBackgroundJobs(): Array<{
  kind: BackgroundJobKind;
  projectId: string;
  label: string;
}> {
