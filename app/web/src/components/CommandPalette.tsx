            ...
            <Command label="Command Menu" className="flex flex-col">
              ...
              <Command.List className="max-h-[52vh] overflow-y-auto p-2">
                ...
                <Group heading="Actions">
                  <Item tipId="global.sidebarLibrary" onSelect={() => go("/")}>Go to Library</Item>
                  <Item tipId="global.sidebarHelp" onSelect={() => go("/help")}>Open Help Guide</Item>
                  <Item tipId="global.themeToggle" onSelect={() => { const t = getTheme() === "dark" ? "light" : "dark"; setTheme(t); setOpen(false); }}>
                    Toggle theme
                  </Item>
                </Group>
...
function Item({ children, onSelect, value, tipId }: { children: React.ReactNode; onSelect: () => void; value?: string; tipId?: ToolTipId }) {
  const item = (
    <Command.Item
      value={value}
      onSelect={onSelect}
  ...
  return tipId ? <ToolTip id={tipId}>{item}</ToolTip> : item;
}
