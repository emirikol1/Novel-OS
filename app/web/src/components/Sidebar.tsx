...
import { ManuscriptFontSizeControls } from "./ManuscriptControls";
  ...
  return (
    <aside className="flex w-[244px] shrink-0 flex-col bg-ink text-[#c8cedd]">
      ...
      <nav className="flex flex-col gap-0.5 px-3">
        <ToolTip id="global.commandPalette">
          <button
            onClick={() => window.dispatchEvent(new KeyboardEvent("keydown", { key: "k", metaKey: true }))}
            className="mb-1 flex w-full items-center justify-between rounded-lg px-3 py-2 text-[13px] text-[#aab2c4] transition-colors hover:bg-ink-800"
          >
            <span>Search…</span>
            <kbd className="rounded border border-ink-line px-1.5 py-0.5 text-[10px] text-[#9aa3b8]">⌘K</kbd>
          </button>
        </ToolTip>
        <SideLink to="/" label="Library" hint="All projects" tipId="global.sidebarLibrary" />
        <SideLink to="/help" label="Help" hint="Workflow guide" tipId="global.sidebarHelp" />
      </nav>
      ...
      <div className="mt-auto px-5 pb-5 pt-6">
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          <p className="text-[10.5px] tracking-wide text-[#8b93a8]">v{APP_VERSION} · local</p>
          <ManuscriptFontSizeControls />
        </div>
      </div>
    </aside>
  );
