...
User manuscripts, outlines, character bios, and story bible content are **private**. When developing Novel OS or debugging the app:

- **Never** read, list, search, or quote anything under `projects/` — including folder names, slugs, titles, or API bodies that identify the user's work. The log rule change does not change this.
- **Do** read sanitized operational logs under `logs/` and terminal output when troubleshooting.
- Operational logs must not contain prose — use `core/log_redaction.py` (`safe_log`, `size_ref`, `SanitizingLogFilter`).
- See workspace rule `.cursor/rules/story-data-privacy.mdc`.
