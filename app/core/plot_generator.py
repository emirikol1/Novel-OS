...
from context_resolver import BUDGET_PLOT_SUPPORT, format_bible_context_block, resolve_bible_context
from llm_client import LLMClient, LLMError
from state_manager import PlotThread, StoryState
from state_parser import extract_block, parse_fields, _as_list


def collect_bible_supporting_context(state: StoryState, *, hint_text: str = "") -> str:
    """Story bible excerpts for enrichment — not for inventing new plot threads."""
    resolved = resolve_bible_context(state, budget=BUDGET_PLOT_SUPPORT, hint_text=hint_text)
    return format_bible_context_block(
        resolved,
        empty_message="(Story bible is empty — rely on the plot thread and subplots only.)",
    )
