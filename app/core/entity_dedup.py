...
def find_subplot_in_state(
    ...
    *,
    ...
    """Locate a matching subplot line on any plot thread."""
    for thread in state.plot_threads.values():
        if exclude_parent_id and thread.id == exclude_parent_id:
            continue
        idx = match_subplot_index(thread.subplots, query, min_score=min_score)
        if idx is not None:
            return thread.id, idx
    return None


def append_unique_list_entry(
    items: List[str],
    text: str,
    *,
    match_score_fn,
    ...
    """Append text when no similar entry exists. Returns True if appended."""
    text = (text or "").strip()
    if not text:
        return False
    for existing in items:
        ...
        if not existing_text:
            continue
        if match_score_fn(existing_text, text) >= min_score:
            return False
    items.append(text)
    return True
...
def _apply_plot_panel_issue(state: "StoryState", issue: PlotPanelIssue) -> List[str]:
    ...
    if issue.kind == "duplicate_subplot_within":
        ...
        for loc in locs:
            ...
            log.append(f'Removed duplicate subplot on {issue.suggested_parent_id} ({size_ref(removed)})')
        ...
        for loc in sorted(issue.locations, key=lambda x: x["index"], reverse=True):
            ...
            if 0 <= idx < len(parent.subplots):
                ...
                log.append(
                    f'Removed duplicate subplot from {loc["parent_id"]} '
                    f'(kept on {issue.suggested_parent_id}): {size_ref(removed)}',
                )
                # One copy per resolve so authors can delete a single duplicate safely.
                break
        ...
        if issue.kind == "thread_under_wrong_parent" and child.thread_type == "main":
            ...
            for loc in sorted(issue.locations, key=lambda x: x["index"], reverse=True):
                ...
                if 0 <= idx < len(parent.subplots):
                    ...
                    log.append(
                        f'Removed duplicate subplot from {loc["parent_id"]} '
                        f'(kept main thread {child.id}): {size_ref(removed)}',
                    )
            return log
        ...
        for loc in sorted(issue.locations, key=lambda x: x["index"], reverse=True):
            ...
            if idx is not None and subplot_head(parent.subplots[idx]).lower() == child.name.lower():
                removed = parent.subplots.pop(idx)
                log.append(f'Removed redundant subplot line after nesting: {size_ref(removed)}')
