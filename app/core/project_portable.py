...
Format matches project backups (outputs/ + assets/ + db_export.json) with an optional
...
from project_archive import iter_project_archive_files, restore_project_tree, safe_extract_zip
...
def build_package_bytes(
    ...
    *,
    ...
    """Build a portable .zip containing outputs/, assets/, and DB export."""
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for path, arc in iter_project_archive_files(project_dir):
            zf.write(path, arc)
    return buf.getvalue()
...
def import_package_bytes(
    ...
    *,
    ...
    try:
        ...
        db_data, _outputs_src = validate_package(extract_root)
        title = _read_title(extract_root, db_data)
        if preferred_project_id and not (projects_root / preferred_project_id).exists():
            project_dir = projects_root / preferred_project_id
        else:
            project_dir = allocate_project_folder(projects_root, title, slugify)
        project_dir.mkdir(parents=True)
        restore_project_tree(extract_root, project_dir)

        from state_manager import StoryState

        StoryState(str(project_dir), persist_migration=True)
