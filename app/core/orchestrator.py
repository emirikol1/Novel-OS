...
"""
    ...
    def list_plot_threads(self):
        ...
        for thread in sorted(threads, key=lambda t: -t.priority):
            ...
            print(f"  {status_icon} [{thread.priority}] {thread.id} ({thread.thread_type})")
    ...
    def plan_chapter(self, chapter_number: int, summary: str = "", pov: str = "", dry_run: bool = False):
        ...
        if pov:
            ...
            from story_graph import apply_brief_pov_to_chapter  # noqa: WPS433
            apply_brief_pov_to_chapter(self.state, chapter, brief)

        chapter.status = 'planned'
        self.state.save_state()

        prompt = self._generate_chapter_outline_prompt(chapter)
        notes_text = "\n".join(chapter.plot_advances or [])
        if notes_text.strip():
            mention_block = mention_context_for_prompts(notes_text, self.state)
            if mention_block:
                prompt = mention_block + prompt
...
POV and target length are defined in the **Chapter Brief** when present — do **not** include POV or
word-count metadata in the outline output.
...
"""
    ...
    def _build_scribe_user_prompt(self, chapter: ChapterState) -> str:
        """Full Scribe user prompt: chapter header, shared context, optional beat sheet."""
...
"""
    ...
    def _generate_chapter_prompt(self, chapter: ChapterState) -> str:
        ...
        return self._build_scribe_user_prompt(chapter)
    ...
    def write_chapter(self, chapter_number: int, draft_text: str = "", dry_run: bool = False):
        ...
        # Build Scribe user prompt: always include brief + context; beat sheet when outline exists.
        user_prompt = self._build_scribe_user_prompt(chapter)

        mention_block = mention_context_for_prompts(user_prompt, self.state)
        if mention_block:
    ...
    def edit_chapter(
        ...
        mention_block = mention_context_for_prompts(source_text, self.state)
        if mention_block:
            edit_prompt = mention_block + edit_prompt
...
"""
        ...
        hint = "\n".join(
            part for part in (instructions, draft_text[:4000]) if (part or "").strip()
        )
        brief_block = build_scribe_context_block(
            self.state,
            chapter,
            hint_text=hint,
            outline_text=outline_text or None,
...
- Deepen conflict
"""
        }.get(mode, "- General line editing") + author_notes + """
...
"""
    ...
    def validate_chapter(self, chapter_number: int, dry_run: bool = False):
        ...
        findings = run_continuity_checks(
            self.state, self.project_path, as_of_chapter=chapter_number, chapter_text=chapter_text,
        )
        if findings:
        ...
        mention_block = mention_context_for_prompts(chapter_text, self.state)
        if mention_block:
            validation_prompt = mention_block + validation_prompt
        if findings:
...
{brief_block}## Chapter Text to Validate
...
"""
    ...
    def _has_stage_text(self, path: Path) -> bool:
        return path.exists() and bool(path.read_text(encoding="utf-8").strip())

    def _auto_promote_draft_if_no_revised(self, chapter_number: int) -> bool:
        """When approving without a revision pass, seed Final from Draft."""
        nnn = f"{chapter_number:03d}"
        final_path = self.manuscript_dir / f"chapter_{nnn}_final.md"
        revised_path = self.manuscript_dir / f"chapter_{nnn}_revised.md"
        draft_path = self.manuscript_dir / f"chapter_{nnn}_draft.md"
        if self._has_stage_text(final_path) or self._has_stage_text(revised_path):
            return False
        if not self._has_stage_text(draft_path):
            return False
        draft_text = draft_path.read_text(encoding="utf-8")
        final_path.parent.mkdir(parents=True, exist_ok=True)
        tmp = final_path.with_suffix(".md.tmp")
        tmp.write_text(draft_text, encoding="utf-8")
        ...
        if chapter:
            ...
            self.state.save_state()
        print("   📄 Promoted Draft → Final (no revised version)")
        return True

    def approve_chapter(self, chapter_number: int) -> bool:
        ...
        self.state.save_state()
        self._auto_promote_draft_if_no_revised(chapter_number)
...
Examples:
  ...
  %(prog)s import story --chapters-dir ~/my-novel/chapters --title "My Novel" --genre Fiction
    ...
    import_ebook = import_sub.add_parser(
        'ebook',
        help='Import a .txt, .epub, or auto-split monolithic text into a project',
    )
    import_ebook.add_argument(
        '--source', required=True,
        help='Path to .txt, .epub, or a chapters directory',
    )
    import_ebook.add_argument('--project-dir', default='.',
                              help='Novel OS project directory (default: current)')
    import_ebook.add_argument('--title', default='', help='Override detected title')
    import_ebook.add_argument('--genre', default='', help='Required when creating a new project')
    import_ebook.add_argument('--author', default='', help='Override detected author')
    import_ebook.add_argument('--split', default='auto',
                              choices=['auto', 'heading', 'parts', 'single'],
                              help='Chapter split strategy for monolithic text')
    import_ebook.add_argument('--parts', type=int, default=None,
                              help='Split into N equal word-count parts (overrides auto-detect)')
    import_ebook.add_argument('--from-chapter', type=int, default=None,
                              help='Only import from this chapter number')
    import_ebook.add_argument('--to-chapter', type=int, default=None,
                              help='Only import through this chapter number')
    import_ebook.add_argument('--no-extract', action='store_true',
                              help='Copy text only; skip LLM extraction')
    import_ebook.add_argument('--dry-run', action='store_true',
                              help='Save prompts only; do not call the LLM')
    import_ebook.add_argument('--synthesize', action='store_true',
                              help='After import, generate outputs/outline.json from extracted state')
    import_ebook.add_argument('--no-profiles', action='store_true',
                              help='Skip writing outputs/characters/*.md profiles')
    import_ebook.add_argument('--parse-only', action='store_true',
                              help='Print detected metadata and chapter plan; do not import')
    ...
    if args.command == 'import' and getattr(args, 'import_command', None) == 'ebook':
        if getattr(args, 'parse_only', False):
            needs_llm = False
        elif getattr(args, 'no_extract', False):
            needs_llm = bool(getattr(args, 'synthesize', False)) and not getattr(args, 'dry_run', False)
        else:
            needs_llm = not getattr(args, 'dry_run', False)
    ...
    if args.command == 'init':
        ...
        if args.import_command == 'story':
            ...
            if not args.no_profiles:
                ...
                safe_log(f"Wrote {n} character profile(s) to outputs/characters/")
            safe_log("Import complete. Open project in Novel-OS UI or run: status")
        elif args.import_command == 'ebook':
            ...
            if args.parse_only:
                ...
                safe_log(f"Title: {parsed.metadata.title or '(unknown)'}")
                safe_log(f"Author: {parsed.metadata.author or '(unknown)'}")
                safe_log(f"Format: {parsed.metadata.source_format}")
                safe_log(f"Split strategy: {parsed.metadata.split_strategy}")
                for ch in parsed.chapters:
                    wc = len(ch.text.split())
                    safe_log(
                        f"  Ch {ch.number}: {ch.title!r} — {wc} words"
                    )
                return
            ...
            state_file = proj / "outputs" / "state" / "story_state.json"
            if not state_file.exists():
                parsed_preview = parse_ebook(src, split_strategy=args.split, parts=args.parts)
                resolved_title = args.title or parsed_preview.metadata.title
                if not resolved_title or not args.genre:
                    print("❌ New project requires --genre (title can be auto-detected)")
                    sys.exit(1)
                proj.mkdir(parents=True, exist_ok=True)
                orchestrator.init_project(resolved_title, args.genre, args.author or parsed_preview.metadata.author)
            ...
            safe_log(
                f"Imported {summary['chapters_imported']} chapters "
                f"({summary['total_words']} words) via {summary['split_strategy']}"
            )
            if args.synthesize:
                from import_pipeline import ImportPipeline
            ...
            if not args.no_profiles:
                ...
                n = pipe.write_character_profiles()
                safe_log(f"Wrote {n} character profile(s) to outputs/characters/")
            safe_log("Ebook import complete. Open project in Novel-OS UI or run: status")
        else:
            import_parser.print_help()
