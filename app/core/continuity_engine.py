...
def check_unresolved_mentions(
    state: "StoryState",
    text: str,
    chapter_number: Optional[int] = None,
) -> List[Finding]:
    """Flag broken character/lore mentions in chapter text (non-blocking)."""
    ...
    out: List[Finding] = []
    for rm in resolve_mentions_in_text(text, targets):
        if not rm.broken:
            continue
        kind_label = "character" if rm.kind == "char" else "lore"
        out.append(Finding(
            severity="warning",
            category="unresolved_mention",
            message=f"Unresolved {kind_label} mention {rm.raw!r} — no matching cast/bible entry.",
            suggestion="Fix the label, add the entity to cast/story bible, or remove the mention.",
            chapter=chapter_number,
        ))
    return out


def check_mention_state_conflicts(
    state: "StoryState",
    text: str,
    chapter_number: Optional[int] = None,
) -> List[Finding]:
    """Obvious conflicts between resolved mentions and character memory."""
    if not text or "[[" not in text:
        return []
    from mentions import build_mention_targets, resolve_mentions_in_text, strip_mentions  # noqa: WPS433

    targets = build_mention_targets(state)
    plain = strip_mentions(text).lower()
    out: List[Finding] = []
    for rm in resolve_mentions_in_text(text, targets):
        ...
        es = (char.emotional_state or "").lower()
        notes = (char.notes or "").lower()
        if any(k in es or k in notes for k in DEAD_KEYWORDS):
            # Active on-page cues while memory says dead
            if re.search(rf"\b{re.escape(char.full_name)}\b", plain, re.I):
                out.append(Finding(
                    severity="warning",
                    category="mention_dead_conflict",
                    message=(
                        f"{char.full_name} is mentioned in ch{chapter_number or '?'} prose but "
                        f"story memory flags them dead/killed."
                    ),
                    suggestion="Confirm flashback/rumor vs. update character state after review.",
                    chapter=chapter_number,
                    entity_id=char.id,
                ))
        loc = (char.current_location or "").strip()
        if loc and len(loc) > 2:
            # Very loose: mention label equals a different location string in prose near name
            loc_l = loc.lower()
            if loc_l not in plain and rm.label.strip().lower() != loc_l:
                if f"at {loc_l}" in plain or f"in {loc_l}" in plain:
                    continue
                # Only flag when mention label looks like a place contradicting stored location
                if rm.label.strip().lower() != char.full_name.strip().lower():
                    if rm.label.strip().lower() in plain and loc_l not in plain:
                        out.append(Finding(
                            severity="info",
                            category="mention_location_hint",
                            message=(
                                f"Lore/place mention {rm.raw!r} appears while {char.full_name} "
                                f"memory lists location '{loc}'."
                            ),
                            suggestion="Review location after reading the scene; apply memory updates manually if needed.",
                            chapter=chapter_number,
                            entity_id=char.id,
                        ))
    return out
...
def run_all(state: "StoryState", project_path: Optional[Path] = None,
            ...
            chapter_text: Optional[str] = None) -> List[Finding]:
    ...
    if chapter_text:
        out.extend(check_unresolved_mentions(state, chapter_text, as_of_chapter))
        out.extend(check_mention_state_conflicts(state, chapter_text, as_of_chapter))
    return out
