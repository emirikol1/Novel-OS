...
def test_duplicate_subplot_across_parents(tmp_path):
    ...
    kept = state.plot_threads[issue.suggested_parent_id]
    assert len(kept.subplots) == 1


def test_resolve_across_removes_one_duplicate_at_a_time(tmp_path):
    initialize_project(str(tmp_path), "Test", "Fiction")
    state = StoryState(str(tmp_path))
    _add_plot(state, "plot_a", "Arc A", subplots=["Shared beat: first wording"])
    _add_plot(state, "plot_b", "Arc B", subplots=["Shared beat: second wording"])
    _add_plot(state, "plot_c", "Arc C", subplots=["Shared beat: third wording"])

    issues = find_plot_panel_issues(state, min_score=0.78)
    issue = next(i for i in issues if i.kind == "duplicate_subplot_across")
    before = sum(len(t.subplots) for t in state.plot_threads.values())
    resolve_plot_panel_issue(state, issue.issue_id, issues=issues)
    after = sum(len(t.subplots) for t in state.plot_threads.values())
