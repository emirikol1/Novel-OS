...
def test_cancel_active_aborts_registered_handler():
    q = LLMRequestQueue(max_concurrent=1)
    started = threading.Event()
    release = threading.Event()
    ...
    def hold():
        with q.acquire("Active") as slot:
            started.set()
            slot.register_abort(lambda: aborted.set())
            release.wait(timeout=3)
    ...
    active_id = q.status()["active_items"][0]["id"]
    q.cancel_active(active_id)
...
def test_cancel_for_job_removes_queued_waiter():
    q = LLMRequestQueue(max_concurrent=1)
    from llm_call_context import llm_job_context  # noqa: E402
    ...
    def wait_for_slot():
        try:
            with llm_job_context("Queued job", meta={"kind": "write", "job_id": "job-abc"}):
                with q.acquire("Waiting"):
                    pass
        except Exception as e:
            errors.append(type(e).__name__)
    ...
    assert q.cancel_for_job("job-abc") >= 1
    t2.join(timeout=2)
    assert "QueueCancelledError" in errors
